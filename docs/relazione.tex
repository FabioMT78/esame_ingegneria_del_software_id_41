...
\begin{description}
  \item[web/interface] ricezione delle richieste HTTP, controlli trasversali sull'input, validazione formale, serializzazione e traduzione degli errori;
...
Le route \texttt{/api} attraversano inoltre \texttt{SicurezzaInputHttpMiddleware} prima dei controller. Il middleware ispeziona path, query string e body delegando a \texttt{SicurezzaInputService}, indipendente da Express, la rilevazione ricorsiva di un insieme limitato di pattern sospetti riconducibili a XSS e SQL injection. Un input riconosciuto viene rifiutato con HTTP 400 e produce un log tecnico con valore reso sicuro per la registrazione.

Questa rilevazione è una difesa aggiuntiva e non sostituisce le protezioni contestuali: i singoli input continuano a essere validati secondo il loro significato, le query PostgreSQL rimangono parametrizzate e il contenuto HTML viene sottoposto a escaping nel punto di rendering. Le espressioni regolari usate dal controllo trasversale sono volutamente euristiche e possono produrre falsi positivi o falsi negativi; nello scope didattico della versione 1.0 forniscono una barriera centralizzata, semplice e direttamente testabile.
...
Nel boundary HTTP, \texttt{SicurezzaInputHttpMiddleware} ha la responsabilità di intercettare la richiesta e tradurre una rilevazione sospetta nel normale flusso di errore Express, mentre \texttt{SicurezzaInputService} contiene soltanto la logica di ispezione e classificazione dei pattern. La separazione evita di legare la logica di rilevazione al framework e ne consente il test unitario.
...
\begin{itemize}
  ...
  \item rilevazione trasversale degli input sospetti, con test unitari del \texttt{SicurezzaInputService} e test HTTP del middleware;
  \item scenario end-to-end di UC-02 attraverso HTTP, application layer e PostgreSQL.
\end{itemize}

I test di sicurezza verificano sia payload riconducibili alle famiglie di pattern XSS/SQL injection previste, sia valori legittimi che non devono essere bloccati, oltre all'analisi ricorsiva di oggetti e array e al comportamento HTTP 400 con logging tecnico.
...
\begin{itemize}
  ...
  \item controllo trasversale degli input basato su pattern euristici come difesa aggiuntiva, senza sostituire query parametrizzate, validazione specifica ed escaping contestuale;
