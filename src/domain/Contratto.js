class Contratto {
  ...
  constructor({ dal, tipologia, canoneMensile, giornoPagamento }) {
    if (
  ...
  #checkPeriodoDiCompetenza(dataDiCompetenza) {
    if (dataDiCompetenza.getUTCFullYear() < this.dal.getUTCFullYear() || dataDiCompetenza.getUTCFullYear() > this.al.getUTCFullYear()) {
      throw new RangeError(
        "L'anno passato non è compreso nel periodo del contratto",
      );
    }
    if (dataDiCompetenza.getUTCFullYear() == this.dal.getUTCFullYear() && dataDiCompetenza.getUTCMonth() < this.dal.getUTCMonth()) {
      throw new RangeError(
        "Il mese passato non è compreso nel periodo del contratto",
      );
    }
    if (dataDiCompetenza.getUTCFullYear() == this.al.getUTCFullYear() && dataDiCompetenza.getUTCMonth() > this.al.getUTCMonth()) {
      throw new RangeError(
        "Il mese passato non è compreso nel periodo del contratto",
      );
    }
   ...
   */
  #calcolaImporto(anno, mese, canoneMensile, inizioRapporto = true) {
    const giorniDelMese = new Date(Date.UTC(anno, mese, 0));
    const canoneGiornaliero = canoneMensile / giorniDelMese.getUTCDate();
    const numeroGiorniDaPagare = inizioRapporto ? giorniDelMese.getUTCDate() - this.dal.getUTCDate() + 1 : this.al.getUTCDate();

    return Math.round((canoneGiornaliero * numeroGiorniDaPagare) * 100) / 100;
  }

  calcolaImportoCompetenza(anno, mese) {
    ...
    if (dataDiCompetenza.getUTCMonth() == this.dal.getUTCMonth() && dataDiCompetenza.getUTCFullYear() == this.dal.getUTCFullYear()) {
      return this.#calcolaImporto(anno, mese, this.canoneMensile, true);
    }

    if (dataDiCompetenza.getUTCMonth() == this.al.getUTCMonth() && dataDiCompetenza.getUTCFullYear() == this.al.getUTCFullYear()) {
